...
## Claude: 인포스틸러 악성코드 세션 탈취 경고

Anthropic이 사용자 PC에 감염된 인포스틸러 악성코드(Vidar, Lumma, StealC, RedLine, Atomic Stealer)가 활성 Claude 세션 쿠키를 탈취해 비밀번호나 2FA 없이 유료 사용량을 소진시키고 있다고 경고했다([BleepingComputer](https://www.bleepingcomputer.com/news/artificial-intelligence/anthropic-warns-infostealer-malware-is-hijacking-claude-sessions-to-drain-usage/)). Anthropic 시스템 자체는 침해되지 않았으며, 피해 사용자에 대해 세션 로그아웃·결제수단 초기화·환불을 진행 중이다([SecurityWeek](https://www.securityweek.com/anthropic-warns-claude-users-of-infostealer-malware-infections/)). 별도로, Claude for Teachers가 K-12 학교·학군 대상 무료 엔터프라이즈 솔루션으로 출시되었다([Anthropic Newsroom](https://www.anthropic.com/news)).
